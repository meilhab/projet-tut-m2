...
Ticc~\protect\footnote{Tool for Interface Compatibility and Composition}~\cite{siteTicc, ticcDocumentation} est un outil qui permet de définir des interfaces (appelées modules) et de les composer ensembles. Il est écrit en OCaml et est disponible gratuitement.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Avantage : la justesse des modèles et le respect des spécifications peuvent être vérifiés. 
...
\subsubsection{Exemples concernant les fichiers}
...
\subsubsection{Exemple abordant la compatibilité}
...
On peut remarquer que dans l'exemple de la figure~\ref{Detecteur de Fumee}, que les deux automates d'interfaces sont incompatibles car l'un produit un événement \enquote{FD!} alors que l'autre non.

\begin{figure}[!ht]
	\centering 
	\includegraphics[scale=0.5]{detecteurFumee.png}
