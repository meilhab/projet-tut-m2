...
\section{Utilisation du langage ATL}
...
Les ensembles de messages pris en compte sont :
\begin{itemize}
    \item \textbf{alt} : messages multiples alternatifs (si alors sinon);
    \item \textbf{loop} : messages s'exécutant plusieurs fois (boucle);
    \item \textbf{seq} : messages n'ayant pas d'ordres particuliers.

\end{itemize}

Un message est composé d'un émetteur et d'un récepteur. Il possède aussi une méthode
